🎯 Critical points by rank deficiency
***************************************
...
:math:`f` concatenated :math:`\pi`'s coefficient row is rank-deficient. i.e. it has a nonzero
null vector.
...
:math:`J(f)` has rank :math:`2` where the curve is smooth, so its kernel is a (1-dimensional) tangent
line.  A linear projection :math:`\pi(x)=\pi_0 x+\pi_1 y+\pi_2 z` is critical exactly when its
gradient row :math:`J(\pi)=(\pi_0,\pi_1,\pi_2)` lies in the row space of :math:`J(f)` -- equivalently,
the matrix

.. math::

   M = \begin{bmatrix} J(f) \\ J(\pi) \end{bmatrix} \in \mathbb{C}^{3\times 3}

is **rank-deficient**: there is a nonzero :math:`v` with :math:`M v = 0`.  Introduce that null
...
equations in the six unknowns :math:`x,y,z,v_0,v_1,v_2`.  The null vector :math:`v`
is the curve's tangent direction at a critical point.  Degree stays low (the determinant is
...
Building and solving the critpoint system
===========================================

Writing the criticality statement is three lines of code: differentiate the curve into a Jacobian, stack the
random projection's coefficient row on top, and dot the result with a null vector.  ``bertini``
differentiates symbolically (:func:`bertini.jacobian`), :func:`bertini.random_matrix` draws a
...
.. testcode::
    ...
    M = np.vstack([J, bertini.coefficients(pi)])          # 3 x 3: J(f) stacked over the projection
    v = np.array(bertini.variables('v', 3), dtype=object) # the null-vector unknowns
    v_patch_coeffs = bertini.random_matrix(1, 3, symbolic=True)
...
Each circle is a degree-2 curve, and it has a predictable pair of critical points for
...
are ones we can trivially write down, so they'll be our check -- all four must turn up among the solutions:
...
The solve returns more than those four.  The real critical points come in two flavours:

* **projection-critical points** -- on a *single* component where the curve is smooth, the
  genuine "the projection is tangent" points (two on each circle, and some on the quartic);
* **singular crossings** -- where two components meet (line-meets-circle, circle-meets-quartic), the
  curve is *not* smooth, so :math:`J(f)` is already rank-deficient there and the point satisfies the
  system automatically.  These show up regardless of the projection.
...
and those two collide into one (the fiber goes tangent); push past and there are zero.  The
